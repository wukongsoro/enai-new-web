...
const BLOG_OG_IMAGE = buildAbsoluteUrl('/enai-founder.jpeg');
const FEATURED_BLOG_SLUG = "founder-enterprise-autonomous-governance";
...
export const metadata: Metadata = {
  title: "ENAI Blog | Governed Revenue Execution for Complex Markets",
  description: "Founder notes and industry playbooks on governed revenue execution for Industrials, Professional Services, Private Equity, Financial Services, Software, Hardware, AI-Native, Sovereign AI, Logistics, Healthcare, and Wholesale markets.",
  keywords: [
    ...defaultKeywords,
    "ENAI for industrials",
    "ENAI for professional services",
    "ENAI for private equity",
    "ENAI for financial services",
    "ENAI for software companies",
    "ENAI for hardware companies",
    "ENAI for AI-native companies",
    "ENAI for sovereign AI",
    "governed revenue execution by industry",
  ...
  openGraph: {
    title: "ENAI Blog | Governed Revenue Execution for Complex Markets",
    description: "Founder notes and industry playbooks on governed revenue execution across complex B2B markets.",
    url: BLOG_URL,
    type: "website",
    images: [
      {
        ...
        width: 1200,
        height: 630,
        alt: "ENAI founder letter and revenue execution perspectives",
      },
    ],
  },
  twitter: {
    card: "summary_large_image",
    title: "ENAI Blog | Governed Revenue Execution",
    description: "Founder notes and industry playbooks for complex-market revenue execution.",
...
export default function BlogPage() {
  const visibleBlogPosts = LAUNCH_BLOG_SLUGS
    .map((slug) => blogPosts.find((post) => post.slug === slug))
    .filter((post): post is BlogPost => Boolean(post));
  const featuredPost = visibleBlogPosts.find((post) => post.slug === FEATURED_BLOG_SLUG) ?? visibleBlogPosts[0];
  const remainingPosts = visibleBlogPosts.filter((post) => post.slug !== featuredPost?.slug);
                ...
                <Link key={index} href={`/blog/${post.slug}`} className="group block">
                  <article className="relative h-full bg-white rounded-2xl overflow-hidden hover:shadow-xl transition-all duration-500 hover:-translate-y-1">
                    <div className="relative aspect-[16/10] overflow-hidden bg-black/5">
                      <Image
                        src={post.image || "/GauJDBpWEAEJrSq.jpeg"}
                        alt={post.title}
                        fill
                        className="object-cover group-hover:scale-105 transition-transform duration-700"
                      />
